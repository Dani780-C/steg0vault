...
public class Helper {
    ...
    public static void saveFile(Resource resource, String imageBytes) {
        byte[] data = Base64.getDecoder().decode(Helper.removeHeaderBase64(imageBytes).getBytes());
    ...
    public static void removeFile(Resource resource) {
        File stillCoverfile = new File(Constants.LOCAL_DIRECTORY_NAME.getValue() + "/" + Constants.HEAD_FILE_NAME.getValue() + resource.getImageName());
